...
export class Ball {
    ...
    recoverState() {
        ...
        if (this.oldState.collide && this.oldState.collide.point) {
            this.collide.point = Vector.copyPoint(this.oldState.collide.point);
    ...
    checkIgnore() {
        this.ignores = Collide.resetIgnore(this.getPos(), this.ignores, this.collide, this.finish);
    }

    calcCollide() {
        this.checkIgnore();
        let start = this.getPos();
        let collide = Collide.lcheckNextCollide(start, this.dir, this.ignores, this.hit);
        this.collide = collide;
        this.finish = false;
    ...
    update() {
        ...
        if (this.times < this.role.times) {
            ...
            this.dir = l.getReflectNorm(this.dir, BattleDataManager.instance.ldata.isThrough);
            this.passed = 0; // 只有反弹时才需要将pass设置为0
            Vector.assignPoint(this.collide.point, this);
            this.saveState();
            this.calcCollide();
            return true;
        } else {
